...
**1. Why SQL for a recommendation system?**
Because the data is already there and moving it is the expensive part. A Python
recommender over this dataset would pull 1.2 million interaction rows across the
network, rebuild indexes that PostgreSQL already maintains, and produce a model
that then has to be shipped somewhere to be served. Item to item collaborative
filtering is a self join and a normalisation; SQL is a better language for that
than a loop. The database also gives transactions, constraints, a query planner,
and the ability to join the model output against live catalogue state in the
same plan, which is how the already purchased and out of stock filters cost
nothing. The real argument is operational: one system to deploy, back up and
monitor, and the recommendations can never drift from the data they claim to be
derived from.

**2. Why item to item rather than user to user collaborative filtering?**
Item to item is stable and user to user is not. A product's neighbour set moves
slowly, so the matrix can be refreshed on a schedule; a user's taste vector
changes with every click, so a user to user matrix is stale as soon as it is
built. It also scales in the right direction: there are 2,000 products and
10,000 users here, and in any real storefront the gap is far wider. And it
degrades gracefully, because a user with one interaction still gets that item's
neighbours, whereas user to user needs a neighbourhood that a new user does not
have.

**3. How does your similarity calculation work?**
Each product is a vector over users, where the coordinate is that user's decayed
engagement score, log damped so one obsessive viewer cannot dominate. Cosine
similarity is the dot product over the product of the norms. Dividing by both
norms is what removes popularity from the result: without it, the bestseller co
occurs with everything and becomes everybody's neighbour. Jaccard is computed
alongside as a set overlap check, and sharp disagreement between the two usually
means an artefact. Pairs need at least three common users, each user contributes
at most their 50 strongest items, and each product keeps its 50 strongest
neighbours.

**4. Why did you use CTEs?**
Because the algorithm has stages and I wanted the SQL to have the same stages.
`anchors`, `cf_candidates`, `filtered`, `normalised`, `scored`, `ranked` are the
names I would use at a whiteboard. A reviewer can read the pipeline top to
bottom and can run any single stage in isolation to see what it produced. The
alternative is a nest of subqueries that is correct and unreadable, which in a
system whose whole selling point is explainability would defeat the purpose. In
modern PostgreSQL CTEs are inlined by default, so this costs nothing at
execution time.

**5. Why use materialized views?**
The offline and online split. The similarity build aggregates millions of pairs
and takes 30 seconds; a request has a few milliseconds. Materialized views put
the expensive result somewhere the cheap query can join to it, and unlike a
cache they are real relations with real indexes and real statistics, so the
planner can reason about them. The unique index on each one is what allows
`REFRESH CONCURRENTLY`, so a rebuild does not take the shelf offline.

**6. Which indexes improved performance?**
`idx_interactions_user_product`, a covering index on `(user_id, product_id)`
with the timestamp and type as an `INCLUDE` payload, took the per request anchor
lookup from 97 ms to 0.4 ms. The `INCLUDE` is what makes it an index only scan;
without it the planner finds rows in the index and then does random heap I/O
across a 1.2 million row table. `idx_interactions_product_time` on
`(product_id, interaction_timestamp DESC)` took the per product window query
from 25 ms to 0.3 ms, and the column order is the point: equality predicate
first, range predicate second, so the scan terminates early.
`idx_mv_sim_lookup` gave 28x on candidate generation. Full numbers, plans and
index sizes are in `docs/query_optimization.md`.

**7. How would this scale to 100 million users?**
The online path already scales, because it touches a bounded number of rows per
request regardless of table size. The offline path is what breaks, in two
places. The similarity self join is quadratic in items per user, so the activity
cap becomes essential rather than merely helpful, and the job needs partitioning
by product range with the pairs computed independently. Feature aggregation
needs to become incremental, processing only interactions since the last
watermark rather than the whole history, which the `as_of` design already
supports. `user_interactions` needs range partitioning by month so old
partitions can be detached rather than scanned. Beyond that the honest answer is
that the similarity build moves to Spark or an approximate nearest neighbour
index and PostgreSQL serves the result, which is fine: the serving path, the
filtering and the ranking stay exactly where they are.

**8. How did you prevent data leakage?**
Structurally rather than procedurally. There is one cutoff,
`pipeline_state.as_of_timestamp`, read through `rec_as_of()`, and every view,
materialized view and scoring function reads its horizon from it. Nothing calls
`now()`, so even recency is measured against the cutoff. Evaluation moves the
cutoff and rebuilds the whole offline layer, which makes future events invisible
to the entire pipeline at once instead of requiring a `WHERE` clause in every
query that someone could forget. I will add that it is not foolproof: the first
version of the ranker read the order ledger without the cutoff in its exclusion
set, which leaked future purchases into the filter and drove every accuracy
metric to exactly zero. The comparison against the collaborative baseline is
what caught it, which is an argument for always having a baseline.

**9. How do you handle cold start users?**
Three regimes defined in SQL so every consumer agrees on them. A cold user gets
trending, which is decayed popularity multiplied by momentum and weighted by a
Bayesian smoothed rating rather than a raw count. A light user gets their
category preferences blended with popularity. A warm user gets collaborative
filtering. There is no separate code path: cold start changes which sources
contribute candidates, and the same scoring and ranking code runs over whatever
arrives. The evaluation includes cold users rather than filtering them out,
which is why it shows pure collaborative filtering serving zero percent of them
while the hybrid serves all of them at a 14 percent hit rate. Products have a
cold start problem too, handled by falling back to full text content similarity
when a product has no behavioural neighbours.

**10. Why not implement the recommendation engine entirely in Python?**
Data movement and duplication. Python would have to pull the interaction log
out, and then the filtering logic would have to be reimplemented on the serving
side because the model output has no idea what is in stock or what the user
already bought. That reimplementation is where recommenders rot: the batch job
and the API drift, and nobody notices until a customer is shown the toaster they
bought last week. Keeping scoring in SQL means the evaluation harness measures
exactly what the API serves, because both call the same function. I would move
to Python the moment the model stopped being expressible as relational algebra,
for example matrix factorisation or a neural ranker, and even then the candidate
generation and the filtering would stay here.

**11. How would you make recommendations real time?**
Recommendations already are: the online path is a few milliseconds. What is not
real time is the feature and similarity layer, which is as fresh as the last
refresh. The cheap improvement is refreshing more often, which `CONCURRENTLY`
makes safe. The better one is a two tier read: blend the materialized
`mv_user_product_features` with a small query over interactions since the last
refresh, so the current session's clicks influence the next request. Truly
streaming item to item similarity needs incremental co occurrence counts, which
is a job for a stream processor writing back into the matrix. Session based
recency, meaning what the user looked at in the last five minutes, is a
different and usually more valuable signal than a fresher similarity matrix.

**12. How would you A/B test the recommendations?**
The weights already live in `rec_config`, so an arm is a row set rather than a
deploy. I would extend that table with a variant key, hash the user id into a
bucket, and have `get_recommendations` read the weights for that bucket. Every
served shelf gets logged with the variant, the scores and the reason. The metric
is downstream: click through on recommended products, then add to cart, then
revenue per session, with a guardrail on catalogue coverage so a variant cannot
win by collapsing onto bestsellers. Offline metrics choose which arms are worth
testing; they do not decide the winner, because they systematically favour
recommending things users would have found anyway.

**13. What happens if a product becomes unavailable?**
It is filtered at serving time, not at build time. `is_active` is checked in the
ranker's filter stage, so a product withdrawn one second ago stops being
recommended on the next request without waiting for a refresh. It stays in the
similarity matrix, which is correct: it still carries information about what
other products are similar to each other. The dataset has 4 percent of the
catalogue retired with its history intact specifically so this path is exercised
rather than assumed, and a test asserts no inactive product is ever returned. If
a large share of a user's candidates went inactive at once, the `IF NOT FOUND`
fallback guarantees they still get a shelf.

**14. How would you prevent popularity bias?**
First measure it, which this does: `eval_coverage_report` shows 98 percent of
recommendations coming from the head of the demand curve and 18 percent
catalogue coverage. The direct fix is an inverse popularity penalty in the
blend, dividing the candidate score by the log of its popularity, which is the
standard treatment. Cosine normalisation in the similarity matrix already
removes the worst of it, since a raw co occurrence count would make the
bestseller everybody's neighbour. Beyond that: reserve a slot or two on the
shelf for long tail candidates, add a diversity constraint so the top ten cannot
all come from one subcategory, and treat coverage as a guardrail metric in any
A/B test so a variant cannot win by narrowing.

**15. How would you improve this system with machine learning?**
The candidate generation would stay in SQL and the ranking would become a
learned model. Right now the five weights are hand set; with logged impressions
and clicks, a gradient boosted ranker trained on the same five features plus
user and product context would learn them, and would learn interactions the
linear blend cannot express, such as price sensitivity varying by category.
Matrix factorisation or a two tower embedding model would produce better
similarities than cosine over raw engagement, particularly for sparse products.
Session based sequence models would capture order, which this system ignores
entirely. I would keep the hybrid as the fallback and the baseline, keep the
filtering and business rules in SQL where they cannot be bypassed, and keep the
explanation layer, because a learned ranker that cannot say why is a harder
thing to operate.
